...
最短距離はまだあるようです。マスの左上に表示されている小さな数字を以下「インデックス」と呼ぶことにします。

Scene 4 のインデックス1から2の変化では、マスの枠線に変化はありません。これを利用できないでしょうか。
...
インデックス1において、どの矢印キーを押せば右のマスへと移動しないでしょうか。

### ヒント5

↑か↓のキーを押したとき、黒マスに衝突し同じマスが表示されます。これで盤面が短縮されます。

### ヒント6

次は、Scene 6 について考えてみましょう。
...
例えば、① + ⑤に対する入力例である、`↑2←3→3↓4←1↓1←2↑2`は同様の理由により、Scene 6 の解答とはなりません。
